...
directory = ["cyf/create_de"] #hdf5_auswertung_bcat_core_loop_step1"]#"heat_days"#/core/"agent_based"#export"#renovation"#"export_results"#"loop_adjust_number_of_dhw_stand_alone"
        ...
        for key in file_dict:
            ...
            if True:
                ...
                if pyx_file_name2build == "__init__":
                    ...
                    try:
                        if PYTHON_VERSION == 3 : 
                            result = glob.glob(curr_dir1 + "/" + pyx_file_name2build + '*' + fileext)
                            if len(result) == 1:
                                move(result[0],
                                dest_folder + pyx_file_name2build + fileext)
                            else:
                                print('\n############################################\n Multiple files available: %s\n############################################\n' %(str(result)))
                    
                        else:
                            move(curr_dir1 + "/" + pyx_file_name2build + fileext , dest_folder + pyx_file_name2build + fileext) 
                    except:
                        print('\n############################################\n %s not move - prev. %s file might be locked\n############################################\n' %(fileext, fileext))
