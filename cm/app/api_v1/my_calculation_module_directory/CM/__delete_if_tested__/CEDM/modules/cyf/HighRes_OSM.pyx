...
def core_loop_calc_share_lowRes(np.ndarray[DTYPE1_t, ndim=2] HighResM
                        , np.ndarray[DTYPE1_t, ndim=2]  LARGE_BUILDINGS_array
    ...
    cdef np.ndarray[DTYPE1_t, ndim=2] PX_count_Matrix_LowRes = np.zeros((estimated_number_of_lines
                                                                         + 100000, 6), dtype=DTYPE1)
    cdef np.ndarray[DTYPE2_t, ndim=1] Share_Matrix_LowRes = np.zeros((PX_count_Matrix_LowRes.shape[0]), dtype=DTYPE2)  
